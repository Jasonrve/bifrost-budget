...
def create_server() -> MCPServer[object]:
    ...
    @server.tool(
        ...
        description=(
            ...
            "with a Bifrost-recognized virtual key. Authorization-header callers are mapped to a "
            "virtual key via BIFROST_AUTH_EXCHANGE_MAP; explicit virtual_key, x-bf-vk, and "
...
def _resolve_credential(
...
) -> tuple[str, str, Literal["virtual_key"], dict[str, Any]]:
    ...
    if headers:
        ...
        if authorization and authorization.strip():
            ...
            credential = _exchange_authorization_credential(settings, authorization_trace)
            log_event(
                logging.INFO,
                "auth_source_selected",
